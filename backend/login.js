...
export class LoginController {
  ...
  formatPhoneNumber(phone) {
    ...
    if (cleaned.length >= 3) {
      return (
        cleaned.substring(0, 3) +
        (cleaned.length > 3 ? ' ' + cleaned.substring(3) : '')
      );
    }
      ...
      searchTimeout = setTimeout(async () => {
        ...
        data.forEach((clinic) => {
          ...
          item.innerHTML = `
            ...
            <small class="suggestion-clinic-id">
              ID: ${this.escapeHtml(clinic.ClinicID)}
            </small>
            ${
              clinic.Province
                ? `<br><small>${this.escapeHtml(clinic.Province)}</small>`
                : ''
            }
          `;
  ...
  setupPhoneInput() {
    ...
    phoneInput.addEventListener('input', (e) => {
      ...
      if (
        rawValue.length === 10 &&
        this.isValidPhoneNumber(phoneInput.value)
      ) {
  ...
  async submitStaffRegistration() {
    const registerType =
      this.staffDialog.getAttribute('data-register-type');
    ...
    const clinicInput =
      document.getElementById('regClinicName');

    const clinicName = clinicInput.value.trim();

    const clinicId =
      clinicInput.getAttribute('data-selected-id');
    ...
    if (!email || !fullName) {
      regMessage.innerHTML =
        '<strong style="color:#c33;">Please fill in email and full name</strong>';
    ...
    if (!email.includes('@')) {
      regMessage.innerHTML =
        '<strong style="color:#c33;">Please enter a valid email address</strong>';
    ...
    if (
      ...
      regMessage.innerHTML =
        '<strong style="color:#c33;">Please enter a valid South African phone number</strong>';
    ...
    if (!clinicName || !clinicId) {
      regMessage.innerHTML =
        '<strong style="color:#c33;">Please select a clinic from the list</strong>';

      return;
    }

    regMessage.innerHTML =
      '<strong style="color:#069;">Registering...</strong>';
    ...
    if (registerType === 'receptionist') {
      ...
      if (existing) {
        regMessage.innerHTML =
          '<strong style="color:#c33;">This email is already registered as pending receptionist</strong>';
      ...
      if (error) {
        ...
        regMessage.innerHTML = `
          <strong style="color:#3c3;">
            ✅ Receptionist registration complete!
    ...
    if (error) {
      ...
      regMessage.innerHTML = `
        <strong style="color:#3c3;">
          ...
          Please login with Google after approval.
        </strong>
      `;
    ...
    try {
      const { error } =
        await supabase.auth.signInWithOAuth({
          ...
          options: {
            redirectTo:
              `${window.location.origin}/pages/redirect.html?role=${selectedRole}`
          }
        });
  ...
  toggleMode() {
    ...
    this.registerStaffBtn.style.display =
      this.isLogin ? 'block' : 'none';

    this.registerReceptionistBtn.style.display =
      this.isLogin ? 'block' : 'none';
  ...
  attachEventListeners() {
    ...
    this.registerReceptionistBtn?.addEventListener(
      'click',
      () => {
